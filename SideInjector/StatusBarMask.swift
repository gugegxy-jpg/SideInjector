...
/// 等强时那条带的边界与色彩差看得出来；顶端给满值（1.0）也会让这条带更容易被认出。
/// 现在前端取 **0.75**（看得出糊了，又不像一块实心玻璃），后端淡出到 0，下边界与背景无差。
///
...
struct StatusBarMask: View {
    ...
    var body: some View {
        ...
        //   0 ~ 0.58    0.75        —— 状态栏文字所在区间：看得出糊了，但不是实心玻璃；
        //   0.58 ~ 0.84  0.75 → 0.33 —— 过渡；
        //   0.84 ~ 1.00  0.33 → 0    —— 与背景融为一体，下边界看不出来。
        // 这是唯一的强度旋钮。参考档位（顶端 / 中点 / 过渡 / 底，都保持同一形状）：
        //   更实：1.00 / 1.00 / 0.44 / 0     更淡：0.50 / 0.50 / 0.22 / 0
        .mask {
            LinearGradient(
                stops: [
                    .init(color: .black.opacity(0.75), location: 0),
                    .init(color: .black.opacity(0.75), location: 0.58),
                    .init(color: .black.opacity(0.33), location: 0.84),
