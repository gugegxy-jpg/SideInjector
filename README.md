...
- [x] **执行期间不息屏**：流程开始（`.running`）与等待继续（`.paused`）时自动开启，
      完成 / 失败 / 取消 / 空闲时自动恢复。用系统标准的 `UIApplication.isIdleTimerDisabled`
      （不需要权限、不改变锁屏策略）；由 `Model.outcome` 统一驱动（`ScreenAwake.swift`），
      任何退出路径都会恢复，不会出现「流程结束了屏幕还亮着」。注意它不阻止**手动**锁屏 ——
      手动锁屏仍需后台音频保活（`KeepAlive.swift`，配对时已在用）
